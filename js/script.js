new Vue({
	...
	methods: {
		init() {
			...
			if (localData) {
				...
				if (lastFetchTime < last15mins) {
					...
					// Refresh if last fetch was over 1 hr ago
		...
		filterCountries(query) {
			...
			this.filteredCountries = this.countries.filter(country => {
				for (var i = q.length - 1; i >= 0; i--) {
					if (country.Slug.indexOf(q[i].toLowerCase()) != -1) {
						return country;
					}
				}
			});
		...
		setActiveCountryStats() {
			// Get your country
			this.activeCountryStats = this.countries.find(
				country => country.Slug === this.activeCountry
			);
			...
			if (this.activeCountry != '') {
				Array.prototype.scaleBetween = function(scaledMin, scaledMax) {
					...
					return this.map(num =>
						(
							100 -
							((scaledMax - scaledMin) * (num - min)) / (max - min) +
							scaledMin
						).toFixed(2)
					);
				};
				...
				fetch(
					this.url + 'total/country/' + this.activeCountry + '/status/confirmed'
				)
					.then(res => res.json())
					.then(data => {
						if (data.length > 0) {
							...
							this.start =
								start.getDate() + ' ' + this.months[start.getMonth()];
		...
		scaleBetween(unscaled, minAllowed, maxAllowed, min, max) {
			return (
				((maxAllowed - minAllowed) * (unscaled - min)) / (max - min) +
				minAllowed
			);
		},
